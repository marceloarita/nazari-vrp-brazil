# Part I — Reproducing Nazari et al. (2018): Attention-Based Routing with REINFORCE

> Part of a two-part project — see the [project overview](README.md). Continues in [Part II — Learning São Paulo](PART_II.md).
...
Part I reproduces the attention-based policy from [Nazari et al. (2018)](refs/1802.04240v2.pdf) end-to-end — model architecture, REINFORCE training loop, and baseline variants — tests it on two problem sizes (VRP10 and VRP20), and ends with a zero-shot evaluation on real São Paulo delivery data (Olist dataset). The zero-shot failure it exposes — distribution shift — is the subject of [Part II](PART_II.md).
...
Part I isolated the bottleneck: the failure on real data is **distribution shift, not model capacity**. Part II attacks it directly — training per zone on real SP customers, with a better learning signal (POMO) and a 2-opt cleanup — and cuts the gap on the hardest zone from 36.6% to 3.8%.

📄 **[Continue to Part II → Learning São Paulo: Per-Zone Routing](PART_II.md)**

*(Project pipeline and acknowledgements are in the [project overview](README.md).)*
